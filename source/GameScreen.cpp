...
ImportedModel GameScreen::marbleObj = ImportedModel("objects/marble.obj");
ImportedModel GameScreen::boardObj = ImportedModel("objects/board.obj");

GameScreen::GameScreen() : Screen() {
	SoundManager* soundManager = SoundManager::getInstance();
	soundManager->playInLoop("audio/background.mp3");
	soundManager->changeVolume(0.18);
	
	gameController.newGame(BOARD_CONFIGURATION::FULL, DIFFICULTY::EASY);
...
void GameScreen::render(Window& window) {
	lightDirection = glm::normalize(glm::vec4(0, 0, 5, 0)) * camera.getViewMatrix();
...
void GameScreen::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {}
				...
				if (board[(int)pixel[0]][(int)pixel[1]] == 1) {
					...
					if (gameController.getMarble().first != -1) {
						...
						if (gameController.move()) {
							SoundManager* soundManager = SoundManager::getInstance();
							soundManager->playOnce("audio/move.mp3");
						}
					}
				}
			}
...
void GameScreen::displayBoard(GLFWwindow* window, double currentTime) {
	...
	boardShaderProgram.setUniform3fv("lightDirection", lightDirection);
	...
	glBindBuffer(GL_ARRAY_BUFFER, boardVbo[2]);
	...
	for (int y = 0; y < 7; y++) {
		for (int x = 0; x < 7; x++) {
			if (board[y][x] == 1) {
				...
				marbleShaderProgram.setUniformMatrix4fv("mv_matrix", mvMat);
				marbleShaderProgram.setUniformMatrix4fv("proj_matrix", pMat);
				marbleShaderProgram.setUniform3fv("lightDirection", lightDirection);
				...
				glBindBuffer(GL_ARRAY_BUFFER, marbleVbo[2]);
				glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, 0);
				glEnableVertexAttribArray(2);
