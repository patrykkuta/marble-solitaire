...
private:
	ImportedModel marbleObj = ImportedModel("marble.obj");
	ImportedModel boardObj = ImportedModel("board.obj");
	Shader marbleShaderProgram = Shader("marbleVertex.glsl", "marbleFragment.glsl");
	Shader boardShaderProgram = Shader("boardVertex.glsl", "boardFragment.glsl");
	Texture marbleTexture = Texture("marbleTexture.jpg");
	Texture boardTexture = Texture("boardTexture2.jpg");
	Framebuffer offScreenFramebuffer;
	Camera camera = Camera(glm::vec3(0, 0, 4), glm::vec3(60, 0, 0));
	GameController gameController = GameController();

	GLuint marbleVao[1], boardVao[1]; // renderer class?
	GLuint marbleVbo[3], boardVbo[3]; // renderer class?
	int width, height; // renderer class
	float aspect; // renderer class
	glm::mat4 pMat; // renderer class
	...
	}; // pass to the renderer class?

	void drawSceneForColorPicking(); // renderer class?
	void setupVertices(ImportedModel model, GLuint vao[], GLuint vbo[]); // renderer class?
	void displayBoard(GLFWwindow* window, double currentTime); // renderer class?
